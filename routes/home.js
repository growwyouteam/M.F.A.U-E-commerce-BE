...
router.get('/config', async (req, res) => {
    try {
        // Fetch specific standalone collections first
        const heroBanners = await HeroBanner.find({ isActive: true }).sort({ order: 1 });
        const serviceItems = await ServiceItem.find({ isActive: true }).sort({ order: 1 });

        // Fetch generic sections
        // We populate categoryId and productIds deeply to give full data to frontend
